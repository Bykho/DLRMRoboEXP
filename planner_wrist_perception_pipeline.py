...
def point_wrist_cam_at(target_pose, link, cam, offset=np.array([0, 0, 0.10])):
    ...
    R = link_pose.to_transformation_matrix()[:3, :3]
    cam_origin_world = link_pose.p + R @ offset
    T_wc = get_pose_look_at(cam_origin_world, target_pose.p)
    cam.set_local_pose(link_pose.inv() * sapien.Pose.from_transformation_matrix(T_wc))

# -----------------------------------------------------------------------


def make_ee_pose_facing(target_pose, distance=0.10):
    """
    Returns a world‑frame sapien.Pose where the origin sits `distance`
    behind the target along the camera's –Z axis and the z‑axis points
    toward the target (like get_pose_look_at, but for EE).
    """
    tgt = target_pose.p
    # choose a point in front of robot at table height
    eye = tgt + np.array([0.0, -distance, 0.15])   # tweak as needed
    T = get_pose_look_at(eye, tgt)                 # world 4×4
...
try:
    ...
    # try:
    #     print("Initialising MPLib planner ...")

    #     planner = mplib.Planner(
    #         urdf       = robot_urdf_path,
    #         srdf       = "/home/nb3227/RoboEXP/assets/robot/simple_arm/jaco2/jaco2.srdf",
    #         move_group = "j2s7s300_link_7",        # tip link in the SRDF chain
    #         verbose    = True
    #     )
    #     print("✅  MPLib planner initialised")
        
    # except Exception as e:
    #     print(f"Error initializing planner: {e}")
    #     print("Cannot continue without planner. Exiting.")
    #     sys.exit(1)
...
end_link = next(l for l in robot.get_links()
                if l.get_name() == "j2s7s300_end_effector")
...
for step, target_pose in enumerate(poses, 1):
    ...
    # --- assemble for RoboMemory ---
    observations = {
        "wrist": obs
    }
    observation_attributes = {
        "wrist": {
            "pred_phrases": attrs["wrist"]["pred_phrases"],
            "pred_masks":   attrs["wrist"]["pred_masks"],
            "mask_feats":   attrs["wrist"]["mask_feats"],
        }
    }
    object_level_labels = [
        "table",
    ...
    robot_arm_mask = np.zeros((PERCEPTION_H, PERCEPTION_W), dtype=bool)
    if attrs["wrist"]["pred_phrases"] is not None:
        for phrase, m in zip(attrs["wrist"]["pred_phrases"],
                            attrs["wrist"]["pred_masks"]):
            if phrase.startswith("robot"):
                robot_arm_mask |= m
    ...
    robo_memory.update_memory(
        ...
        filter_masks = { "wrist": robot_arm_mask }
    )


    print("\n\n about to visualize memory \n\n")
    if robo_memory.memory_scene:
            vis = o3d.visualization.Visualizer()
            vis.create_window(visible=False)
            pcd = o3d.geometry.PointCloud()
            pts, cols = robo_memory.get_scene_pcd()
            pcd.points = o3d.utility.Vector3dVector(pts)
            pcd.colors = o3d.utility.Vector3dVector(cols)
            vis.add_geometry(pcd)
            vis.poll_events()
            vis.update_renderer()
            vis.capture_screen_image(os.path.join(output_dir, f"step_{step}_memory_snapshot.png"))
            vis.destroy_window()
    else:
        print("⏸  memory_scene is empty, skipping visualize_memory()")
