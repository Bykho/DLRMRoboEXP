#simple_container_demo.py

import argparse
import os
from simple_container_predictor import SimpleContainerPredictor
from scene_visualizer import SceneVisualizer, DummyInstance
...
def analyze_scene(scene_objects, container_type, scene_name=None, api_key=None, visualize=True):
    ...
    print(f"\nPredicted contents:")
    for i, (item, confidence) in enumerate(predicted_contents, 1):
        print(f"  {i}. {item} (confidence: {confidence:.2f})")
    ...
    if visualize:
        ...
        visualizer = SceneVisualizer(output_dir="./output")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Predict container contents and create an action-conditioned scene graph")
    ...
    parser.add_argument("--output-dir", "-d", type=str, default="./output", help="Output directory")
    ...
    # Set the output directory in case it was provided
